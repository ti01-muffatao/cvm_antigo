...
<td align="center"><b>Cod. Treinamento</b></td>
<td><b>Nome Treinamento</b></td>
<td><b>Período</b></td>
<td><b>Ativo</b></td>
</tr>
  <?php $SQL = "select * from t_treinamento where cd_treinamento <> 0 ";
  if($TxPesquisa != ""){$SQL .= " and ds_treinamento like '%". $TxPesquisa."%'";}
  $SQL .= " order by cd_treinamento desc ";
	$RSS = mysql_query($SQL, $conexao);
	while($RS = mysql_fetch_array($RSS)){
	  $Cont = $Cont + 1;
	  if($cor == "#FFFFFF"){$cor = "#DDDDDD";}else{$cor = "#FFFFFF";}?>
	  <tr height="20" bgcolor="<?php echo $cor;?>" id="a<?php echo $Cont?>" onClick="javascript: Clica(this,'<?php echo $RS["cd_treinamento"];?>');" style="cursor:pointer;">
	  <?php
	  echo "<td align='center'>".$RS["cd_treinamento"]."</td>";
	  echo "<td>".strtoupper($RS["ds_treinamento"])."</td>";
	  echo "<td>".date("d/m/Y", strtotime($RS["dt_ini"])). '<b>&nbsp;à&nbsp;</b>'.date("d/m/Y", strtotime($RS["dt_fim"]))."</td>";
	  echo "<td>".$RS["ds_ativo"]."</td>";
	  echo "</tr>";
	}
