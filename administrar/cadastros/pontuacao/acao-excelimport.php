...
$objPHPExcel = $objReader->load("documentos/pontos.xls");
...
for($linha = 1; $linha <= 5000; $linha++){
	if($linha > 1 && utf8_decode($objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0, $linha)->getValue()) != ""){
		...
		if($col1 > 0){
			$SQL = "insert into pontuacao(cd_codigo, cd_pontos, cd_fornecedor, cd_campanha, cd_parcial, cd_grupo, cd_supervisor, cd_gerente)values(" ;
			$SQL .= " '".$col0."', '".$col1."', '".$col2."', '".$col3."', '".$col4."', '".$col5."', '".$col6."' , '".$col7."') ";
