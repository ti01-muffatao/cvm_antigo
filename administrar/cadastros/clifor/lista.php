...
<body style="<?php echo $Fundo;?>">
  ...
  <table width="100%" align="center" border="0" bordercolor="#666666" cellspacing="1" cellpadding="1" style="font-family:verdana; font-size:11px;">
	...
	<?php $SQL = "select * from clifor where cd_clifor > 0 ";
	  if($TxPesquisa != ""){$SQL .= " and ds_razao like '%". $TxPesquisa."%'";}
	  $SQL .= " order by ds_razao asc limit 100 ";
		$RSS = mysql_query($SQL, $conexao);
		while($RS = mysql_fetch_array($RSS)){
			$Cont = $Cont + 1;
			if($cor == "#FFFFFF"){$cor = "#DDDDDD";}else{$cor = "#FFFFFF";}?>
			<tr height="20" bgcolor="<?php echo $cor;?>" id="a<?php echo $Cont?>" onClick="javascript: Clica(this,'<?php echo $RS["cd_clifor"]?>');" style="cursor:pointer;">
			<?php
			echo "<td>".$RS["cd_fornecedor"]."</td>";
			echo "<td>".strtoupper($RS["ds_razao"])."</td>";
			echo "<td>".$RS["ds_observacao"]."</td>";
			echo "</tr>";
		}
